/**
 * genera-listing-de.js
 *
 * Script bulk: genera listing in tedesco per tutti i prodotti con listing IT compilato,
 * usando Claude AI (generateAllAiAttributesDE) e salva in product_attribute_values_de.
 *
 * Uso:
 *   node scripts/genera-listing-de.js              → genera solo i prodotti senza listing DE
 *   node scripts/genera-listing-de.js --force      → rigenera tutti (sovrascrive)
 *   node scripts/genera-listing-de.js --id 42      → solo prodotto con id=42
 *   node scripts/genera-listing-de.js --id 42,43   → solo prodotti 42 e 43
 *
...
const { generateAllAiAttributesDE } = require('../services/anthropicService');
const { saveAiValuesDE, getProductListingDE } = require('../services/attributeService');
...
async function main() {
  ...
  console.log('🇩🇪 Generazione listing DE — avvio');
  ...
  for (let i = 0; i < products.length; i++) {
    ...
    // Controlla se già esiste listing DE (skip in modalità incrementale)
    if (!forceMode) {
      const existingDE = await getProductListingDE(product.id);
      const hasContent = Object.keys(existingDE).some(k =>
        existingDE[k] && existingDE[k].length > 0
      );
      if (hasContent) {
        console.log(`⏭️  ${label} — DE già presente, skip`);
        skipped++;
        continue;
      }
    }
    ...
    try {
      ...
      await saveAiValuesDE(product.id, aiValues);
